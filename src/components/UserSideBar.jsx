...
const UserSideBar = (props) => {
  ...
  const [navBar, setNavBar] = React.useState([
    ...
    { text: "User", icon: <LoginIcon />, path: "/User/Profile" },
  ]);
    ...
    if (!getNav) {
      if (LoginService.isLoggedin()) {
        UserService.getCurrentUser().then((result) => {
          ...
          const prevNavBar = [...navBar];
          prevNavBar[2].text = user.userName;
      ...
      <List>
        <ListItem key={"allQuiz"} disablePadding>
          ...
          >
            ...
            <ListItemText
              primary={
                <span style={{ fontFamily: "poppins" }}>All Categories</span>
              }
            />
          </ListItemButton>
        </ListItem>
        ...
        <ListItem key={"logout"} disablePadding>
          <ListItemButton onClick={() => Logout()}>
            ...
            <ListItemText
              primary={<span style={{ fontFamily: "poppins" }}>Logout</span>}
            />
          </ListItemButton>
        </ListItem>
      </List>
        ...
        <Toolbar>
          <Typography variant="h6" noWrap component="div">
            <div style={{ color: "black" }}>
              <b>
                e<span style={{ color: "blue" }}>X</span>am
              </b>
              portal
            </div>
          </Typography>
          <div
            className="d-flex justify-content-between ms-auto"
            style={{ width: "300px" }}
          >
            {navBar.map((item) => (
              <p
                key={item.text}
                style={{ marginTop: "5px", color: "black", cursor: "pointer" }}
              >
                {item.text}
              </p>
            ))}
