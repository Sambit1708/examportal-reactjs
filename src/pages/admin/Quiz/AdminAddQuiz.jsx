...
import { Box, Button, FormControl, InputLabel, MenuItem, OutlinedInput, Select, Switch, TextField, Typography, alpha, styled } from '@mui/material'
...
export default function AdminAddQuiz() {
  ...
  const createQuiz = async () => {
    try {
      ...
      var newData = {...quiz}
      newData.markPerQuestion = parseInt(newData.maxMark)/parseInt(newData.noOfQuestions)
      setQuiz(newData)
      console.log(newData)
      ...
      <Box sx={{display: 'flex'}}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3, marginTop:'20px', textAlign: "center" }}>
          <div className='container-size container-card p-5'>
            <h5 style={{textTransform: "uppercase", textDecoration: "underline"}}>Add Quiz</h5>
            <form id="list-item" onSubmit={(event) => submitQuizForm(event)}>
              <div className='row'>
                ...
                <TextField
                    ...
                    multiline rows={3} className='mb-3'
                ...
                <div className='row d-flex justify-content-between mb-3'>
                  <TextField
                      label="Maximum Marks" id="maxMark"
                      type="text" variant="filled" className='form-number'
                      onChange={(event) => handleData(event)}
                      inputProps={{
                        style: {fontFamily: 'poppins'}
                      }}
                  ...
                  />
                  <TextField
                      label="Number of Question" type="text"
                      variant="filled" id='noOfQuestions' className='form-number'
                      onChange={(event) => handleData(event)}
                      inputProps={{
                        style: {fontFamily: 'poppins'}
                      }}
