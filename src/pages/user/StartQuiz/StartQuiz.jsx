...
export default function StartQuiz() {
  ...
  const fetchResultByUserAndQuiz = async (id) => {
    try {
      ...
      if (resultResponse.status === 200) {
        Swal.fire(
          "Alredy Appeared",
          "You have already appeared this examination.",
          "warning"
        );
  ...
  const QuizTimer = () => (
    <React.Fragment>
      <Card
        sx={{ position: "fixed", right: "0", width: [100, 150, 200, 300, 300] }}
      >
        <CardContent>
          ...
          <Box textAlign="center">
            ...
            <CircularProgress
              ...
              size={60}
              thickness={5}
              value={(second / (questions.length * 2 * 60)) * 100}
              color={
                Math.floor((second / (questions.length * 2 * 60)) * 100) > 60
                  ? "success"
                  : Math.floor((second / (questions.length * 2 * 60)) * 100) >
                    30
                  ? "warning"
                  : "error"
              }
  ...
  const handleAnswerChange = (event, id) => {
    ...
    const questions = [...newData.questionAnswer];
    let questionIdex = questions.findIndex(
    ...
    if (questionIdex === -1) {
      ...
      questions[questionIdex].choosedAnswer = event.target.value;
    }
    newData.questionAnswer = questions;
